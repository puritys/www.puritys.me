/********Need JQuery
var file_upload=new vjUI_fileUpload({"dragInBox":"#dragBox,.test","loadCallback":previewImage,"uploadUrl":"file.php","completeFunc":complete});
******/
if(!vj_tmp_object){
	var vj_tmp_object=0;
}
...
(function(){
	...
	o.ajaxUpload=function(file){
		...
		vFD.append(this.fileName,file);
		
		var oXHR = new XMLHttpRequest();
		if($.isFunction(this.progressFunc))
			oXHR.upload.addEventListener('progress', this.progressFunc, false);
		if($.isFunction(this.completeFunc))
			oXHR.addEventListener('load', this.completeFunc, false);
